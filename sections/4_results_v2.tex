...
We evaluate on five large-scale chest radiograph benchmarks. \textbf{MIMIC-CXR}~\cite{johnson2019mimiccxr} contains \placeholder{377,110} chest radiographs from \placeholder{65,379} patients with associated radiology reports; we use the official train/validation/test splits stratified by patient to prevent information leakage. \textbf{CheXpert Plus} builds on the CheXpert dataset~\cite{irvin2019chexpert}, and we follow the standard protocol of evaluating on the expert-labeled subset. \textbf{VinDr-CXR} is a multi-center chest radiograph dataset with radiologist-verified annotations, \textbf{RSNA Pneumonia} focuses on pneumonia detection from frontal chest X-rays, and \textbf{NIH Chest X-ray} is a large-scale public collection of chest radiographs. All datasets are labeled or relabeled for 14 thoracic observations using the CheXpert labeler: Atelectasis, Cardiomegaly, Consolidation, Edema, Enlarged Cardiomediastinum, Fracture, Lung Lesion, Lung Opacity, No Finding, Pleural Effusion, Pleural Other, Pneumonia, Pneumothorax, and Support Devices.
...
Our benchmarks fall into two categories: concept bottleneck models (CBMs) and black-box models.

\textbf{CBM benchmarks:} (1)~\textbf{Label-free CBM}~\cite{oikarinen2023labelfree}, which learns concepts without manual labels using CLIP-guided supervision; (2)~\textbf{Post-hoc CBM}~\cite{yuksekgonul2023posthoc}, which retrofits concept bottlenecks onto pretrained models; (3)~\textbf{LaBo CBM}, which introduces a label-only bottleneck formulation; and (4)~\textbf{AdaCBM}~\cite{xu2024adacbm}, an energy-based concept bottleneck model with adaptive concept selection. All CBM baselines use the same visual backbone as \radcbm\ when architecturally compatible.

\textbf{Black-box models:} we treat the vision towers of large vision--language models as black-box baselines. Specifically, we include (1)~\textbf{MedCLIP}~\cite{wang2022medclip}, (2)~\textbf{CXR-CLIP}~\cite{you2023cxrclip}, and (3)~\textbf{CheXzero}, all evaluated by attaching task-specific heads or using zero-shot prompting on the frozen vision encoders.
...
Table~\ref{tab:classification} presents classification performance across CBM and black-box benchmarks. \radcbm\ matches or exceeds the strongest CBM baselines while providing interpretable concept-mediated predictions. On MIMIC-CXR, \radcbm\ attains a macro AUC of \placeholder{0.XXX}, matching DenseNet-121 (\placeholder{0.XXX}) and outperforming all CBM baselines. The hierarchical architecture improves over the flat variant by \placeholder{X.X} percentage points in macro AUC, with notable gains on region-specific pathologies such as Pleural Effusion (\placeholder{+X.X\%}) and Pneumothorax (\placeholder{+X.X\%}).

Vision-language models treated as black-box vision towers (MedCLIP, CXR-CLIP, CheXzero) achieve reasonable zero-shot performance but fall short of supervised CNNs and CBMs, particularly for rare findings. Among CBM approaches, methods relying on small manually curated vocabularies (XCB) or LLM-generated concepts (Yan et al.) exhibit lower classification performance, suggesting that ontology-grounded concept banks with broader coverage provide stronger supervisory signal.

\begin{table*}[t]
\centering
\caption{Classification performance (AUC-ROC) on MIMIC-CXR and CheXpert test sets. Best results in \textbf{bold}, second-best \underline{underlined}. CNN: supervised convolutional backbone; VLM: vision--language model (black-box vision tower); CBM: concept bottleneck model; H-CBM: hierarchical CBM. All concept-based methods use DenseNet-121 backbone where architecturally compatible. Results averaged over 3 seeds; standard deviations $<$0.01 omitted for clarity.}
...
\midrule
Label-free CBM & CBM & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XX} & \placeholder{.XXX} \\
...
\midrule
Label-free CBM & CLIP-guided discovery & \placeholder{$\sim$50} & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \ding{55} \\
Post-hoc CBM & Projection of pretrained features & \placeholder{$\sim$50} & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \ding{55} \\
...
\midrule
\radcbm\ (flat) & RadGraph + UMLS & 1,312 & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \ding{51} \\
\radcbm\ (hier.) & RadGraph + UMLS & 1,312 & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \placeholder{.XXX}$\pm$\placeholder{.XXX} & \ding{51} \\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:region_performance} presents performance decomposed by anatomical region, validating the hierarchical architecture. Region abnormality AUC (computed from pooled concept locations, not standalone region labels) is high across all regions. Region AUCs are computed using surrogate region targets obtained by max-pooling present concepts per region from RadGraph outputs; no manual region labels are used. Lung (\placeholder{0.XXX}) and heart (\placeholder{0.XXX}) show the strongest performance, reflecting the prevalence and visual distinctiveness of pathology in these regions, while pleura (\placeholder{0.XXX}) and mediastinum (\placeholder{0.XXX}) exhibit slightly lower region AUC, consistent with the subtlety of findings in these areas.

Finding-level AUC, measured on concepts within each region, correlates with region abnormality performance: regions with accurate abnormality detection support more reliable finding predictions. The lung region, containing the largest concept vocabulary (\placeholder{142} concepts), achieves finding AUC of \placeholder{0.XXX}, while the smaller bone vocabulary (\placeholder{34} concepts) attains \placeholder{0.XXX}. Per-region finding AUCs use the same report-derived, pooled-from-concept supervision rather than independent manual region annotations. These results confirm that the hierarchical decomposition captures clinically meaningful region-finding relationships.
...
% \midrule
% Label-free CBM & \placeholder{.XX}$\pm$\placeholder{.XX} & --- & --- & --- \\
% Post-hoc CBM & \placeholder{.XX}$\pm$\placeholder{.XX} & --- & --- & --- \\
...
\midrule
\textbf{All concepts} & 1,312 & \placeholder{.XXX} & \placeholder{.XXX} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:generalization} evaluates generalization across datasets by training on one dataset and testing on the other. \radcbm\ exhibits smaller performance degradation than both supervised CNN and black-box VLM baselines when transferring from MIMIC-CXR to CheXpert (\placeholder{$-$X.X\%} vs. \placeholder{$-$X.X\%} macro AUC), suggesting that ontology-grounded concepts provide more transferable representations than end-to-end learned features. The improvement is most pronounced for concepts with consistent visual presentations across institutions (e.g., cardiomegaly, pneumothorax) and smaller for findings whose appearance varies with imaging protocols (e.g., subtle opacities).
...
The experimental results support three main findings. First, \radcbm\ matches the classification performance of both supervised CNNs and black-box VLM baselines while providing interpretable, concept-mediated predictions, demonstrating that the bottleneck constraint does not materially sacrifice diagnostic accuracy. Second, hierarchical organization with multiplicative gating substantially improves concept quality (concept AUC: \placeholder{+X.X} points) and clinical plausibility (implausible activation rate: \placeholder{XX.X\%} $\rightarrow$ \placeholder{X.X\%}) compared to flat architectures. Third, ontology-grounded concept banks derived from RadGraph and UMLS provide broader coverage (1,312 vs. 14 concepts) and better intervention faithfulness than LLM-generated or manually curated alternatives, enabling reliable what-if analysis at the concept level.
