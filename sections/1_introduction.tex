...
Classical CBMs require dense concept labels for every training image, and chest X-ray datasets contain hundreds of thousands of studies with 50--200 relevant findings each.
...
These labels are noisy---reports contain non-visual information, hedged language, and inconsistent phrasing---but they exist at scale, for free.
...
A two-level predictor first estimates whether each region is abnormal, then predicts findings within that region, with multiplicative gating so that findings only activate when their region is flagged.
...
The linear label head is critical.
Because labels are linear functions of concepts, each prediction decomposes exactly into concept contributions, and editing a concept (say, forcing effusion to zero) changes the label in a predictable, auditable way.
